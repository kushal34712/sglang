...
for dirname in glob.glob(os.path.join(dirpath, "trace*")):
    ...
    for filename in tqdm(glob.glob(os.path.join(dirname, "*.json"))):
        ...
        if "prefill" not in filename and "decode" not in filename: continue
    ...
    time[trace_name] = dict(sorted(time[trace_name].items(),
                                   key=lambda x: int(x[0].split("_")[-1])))
    size[trace_name] = dict(sorted(size[trace_name].items(),
                                 key=lambda x: int(x[0].split("_")[-1])))
